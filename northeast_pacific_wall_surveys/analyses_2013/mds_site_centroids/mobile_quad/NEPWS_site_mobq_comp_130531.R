...
# compare abundances vs P/A
# nmds on NEPWS mobile data from quadrats, using mob5.csv - lumped functional groups and taxa.  with DUMMY variable so that i don't have to remove rows without species
...
mds.un <- cmdscale(mat.dist, k=100)
mds.un

summary(mds.un)
plot(mds.un, col=c(quad.env$Region))
names(quad.env)
...
### bind
pc.df.un <- cbind(quad.env, mds.un)
names(pc.df.un)
nrow(pc.df.un)

### now take arithmetic averages of pc's for each site
site.mean.un <- aggregate(pc.df.un[,15:114], list(pc.df.un$Site), mean)
site.mean.un
names(site.mean.un)

### create matrix of pc's
mat.pc.un <- site.mean.un[,2:101]
mat.pc.un
...
### get principal coordinates; use classic (metric) MDS scaling
### what k should i use??? arbitrary choice - 100?
mds.pa <- cmdscale(mat.pa.dist, k=100)
mds.pa

summary(mds.pa)
plot(mds.pa, col=c(quad.env$Region))
names(quad.env)
...
### bind
pc.df.pa <- cbind(quad.env, mds.pa)
names(pc.df.pa)
nrow(pc.df.pa)

### now take arithmetic averages of pc's for each site
site.mean.pa <- aggregate(pc.df.pa[,15:114], list(pc.df.pa$Site), mean)
site.mean.pa
names(site.mean.pa)

### create matrix of pc's
mat.pc.pa <- site.mean.pa[,2:101]
mat.pc.pa
### run nMDS using euclidean distance on this matrix, reduce to two dimensions
